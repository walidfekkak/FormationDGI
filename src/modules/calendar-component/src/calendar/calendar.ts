...
@Component({
    ...
    template: `
    <ion-grid>
        <ion-row class="month-header" justify-content-center>
            <ion-col col-auto (click)="back()">
                <ion-icon ios="ios-arrow-back" md="md-arrow-back"></ion-icon>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </ion-col>
            <ion-col col-auto>
                <div (click)="selectYear()">{{displayYear}} - {{displayMonth + 1 | monthName:lang}}</div>
            </ion-col>
        ...
        <ion-row>
            <ion-col class="calendar-days-row center calendar-header-col" *ngFor="let head of weekHead">{{head}}</ion-col>
        </ion-row>
...
export class Calendar {
    ...
    weekHead: string[] = ['D', 'L', 'M', 'M', 'J', 'V', 'S'];

    constructor(public popoverCtrl: PopoverController, public translateService: TranslateService) {
    ...
    ngAfterContentInit() {
        ...
        if(this.selectedDate){
          ...
          var splittedVal = this.selectedDate.split('-');
            var year = parseInt(splittedVal[0])
            var month =  parseInt(splittedVal[1])-1
            var day =  parseInt(splittedVal[2])
            this.jumpToDate(year, month, day);
        }
    ...
    jumpToDate(year, month, day) {
        ...
        let dayIndex = _.findIndex(this.dateArray, {
            ...
            month: month,
            date: day,
            //isThisMonth: this.currentMonth == month?true:false
        })
        this.lastSelect = dayIndex;
        if(dayIndex >= 0){
            this.dateArray[dayIndex].isSelect = true;
            this.onDaySelect.emit(this.dateArray[dayIndex]);
        }
    }

    selectYear(){
        let popover = this.popoverCtrl.create(YearChooserPopoverPage);
        popover.present({
        });
        popover.onDidDismiss(data => {
            if(!data) return;
            //console.log(data);
            this.displayYear = parseInt(data.year);
            this.jumpToDate(parseInt(data.year), 0, 1);
        });
    }
